...
Dong Chen: ahmedamz@outlook.com
Date: 01/05/2021
"""
import numpy as np
...
class MergeEnvAdv(AbstractEnv):
    """
    A highway merge negotiation environment.

    The ego-vehicle is driving on a highway and approached a merge, with some vehicles incoming on the access ramp.
    It is rewarded for maintaining a high speed and avoiding collisions, but also making room for merging
    vehicles. The enviroment will  include an adversary.
    """
    n_a = 5
    n_s = 25
    ...
    def _collision_reward(self, vehicle: Vehicle, weight: int = 1) -> int:
        """
            returns w_c * r_c
        """
        return weight * (-1 * vehicle.crashed)
    

    def _merge_reward(self, vehicle: Vehicle, weight: int = 1) -> float:
        """
            return w_m * r_m
        """
        # compute cost for staying on the merging lane
        if vehicle.lane_index == ("b", "c", 1):
            # a vehicle that is located at the merge lane
            '''
                ------------------------------------------
                        veh
                ------------------------------------------
               ...
               given 
                    x_g   -> global x location
                    L_g   -> global end of merge
                    B     ->  Before distance
                    C     ->  Converge distance
                    M     -> merge distance
            ...
            L = self.ends[2]  ## equal 100
            L_g = sum(self.ends[:3])  ## equal 420
            x_g = vehicle.position[0]
            Merging_lane_cost = - np.exp(-(x_g - L_g) ** 2 / (10 * L))
        else:
            Merging_lane_cost = 0
        
        return Merging_lane_cost * weight

    def _head_reward(self, vehicle: Vehicle, weight: int = 1) -> float:
        """
            return w_h * r_h
        """
        ...
        return Headway_cost * weight
    def _speed_reward(self, vehicle: Vehicle, weight: int = 1) -> float:
        """
            return w_s * r_s
        """
        # the optimal reward is 0
        scaled_speed = utils.lmap(vehicle.speed, self.config["reward_speed_range"], [0, 1])
        scaled_speed = np.clip(scaled_speed, 0 ,1)

        return  scaled_speed * weight
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
       ...
       """
        # the optimal reward is 0
        scaled_speed = self._speed_reward(vehicle, weight = self.config["HIGH_SPEED_REWARD"])
        # compute cost for staying on the merging lane
        Merging_lane_cost = self._merge_reward(vehicle, weight = self.config["MERGING_LANE_COST"])

        # compute headway cost
        Headway_cost = self._head_reward(vehicle, weight = self.config["HEADWAY_COST"])
        # compute overall reward
        reward = self._collision_reward(vehicle, weight = self.config["COLLISION_REWARD"]) \
                 + scaled_speed \
                 + Merging_lane_cost \
                 + (Headway_cost if Headway_cost < 0 else 0)
        return reward
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
        """
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        # info["regional_rewards"] = sum(vehicle.regional_reward for vehicle in self.controlled_vehicles) / len(self.controlled_vehicles)
    ...
    def _reset(self, num_CAV=0) -> None:
        ...
        if self.config["traffic_density"] == 1:
            ...
            num_CAV = 8
            num_HDV = 0
        elif self.config["traffic_density"] == 6:
            num_CAV = 10
            num_HDV = 0
    ...
    def _make_vehicles(self, num_CAV=4) -> None:
        """
        ...
        # spawn_points_s = [10, 30, 40, 60]
        # spawn_points_m = [5, 25, 50, 75]

        self.adv_lane = np.random.choice(2) 
        adv_point = 0
        if self.adv_lane:
            ## adv is to start in ramp lane
            adv_point = np.random.choice(spawn_points_m)
            spawn_points_m.remove(adv_point)
        else:
            ## adv is to start in straight lane
            adv_point = np.random.choice(spawn_points_s)
            spawn_points_s.remove(adv_point)
        ...
        initial_speed = np.random.rand(num_CAV + 1) * 2 + 25  # range from [25, 27]
        loc_noise = np.random.rand(num_CAV + 1) * 3 - 1.5  # range from [-1.5, 1.5]
        ...
        """spawn the adversary in selected lane"""
        
        lane = road.network.get_lane(("j", "k", 0)) if self.adv_lane else road.network.get_lane(("a", "b", 0))
        self.adv_vehicle = self.action_type.vehicle_class(road, lane.position(
                adv_point + loc_noise.pop(0), 0), speed = initial_speed.pop(0), is_adv = True, env = self)
        if self.add_adv:
            road.vehicles.append(self.adv_vehicle)
        self.adv_vehicle.color = (255, 100 ,100)


    def terminate(self):
        return

    def only_adv_crashes(self):
        for v in self.controlled_vehicles:
            if v.crashed:
                return False
            
        return self.adv_vehicle.crashed
    
    def only_cavs_crashes(self):
        if not self.adv_vehicle.crashed:
            for v in self.controlled_vehicles:
                if v.crashed:
                    return True
        return False
...
class MergeEnvMARLAdv(MergeEnvAdv):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
...
register(
    id='merge-v2',
    entry_point='highway_env.envs:MergeEnvAdv',
)

register(
    id='merge-multi-agent-v1',
    entry_point='highway_env.envs:MergeEnvMARLAdv',
)
