...
class AbstractEnv(gym.Env):
    ...
    def __init__(self, config: dict = None) -> None:
        ...
        # self.ends = [80, 40, 60, 60]  # Before, converging, merge, after
    ...
    def reset(self, is_training=True, testing_seeds=0, num_CAV=0) -> Observation:
        ...
        ## choose if this episode will include an adversary based on the passed ratio
        ...
        if self.config["action_masking"]:
            available_actions = self._get_mask(self.controlled_vehicles)
        else:
            available_actions = [[1] * self.n_a] * len(self.controlled_vehicles)


        ## select the adversary

        if "adv_model" in self.config:
            strat = self.config["train_strat"]
            param = self.config["param"]
            if strat=="homo":
                index = int(param)
                self.adv_model = self.config["adv_model"][index]
            elif strat == "hete":
    ...
    def safety_supervisor(self, actions, adv_action):
        """"
                ...
                for v in [v_fl, v_fr, vehicle, v_rl, v_rr]:
                    ...
                    # other surrounding vehicles
                        ...
                        if type(v) is IDMVehicle:
                            ...
                            if v.is_adv:
                                mdp_controller(v, env_copy, adv_action)
                            else:
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        ## adversay decide on its action
        # adv_model = self.config["adv_model"]

        if self.adv_model:
            adv_state = self.adv_vehicle.get_state().reshape(-1)
            adv_mask = self._get_mask([self.adv_vehicle])
            adv_action = self.adv_model.action(adv_state, adv_mask)
            self.adv_vehicle.act([k for k,v in self.ACTIONS_ALL.items() if v == adv_action][0])
        self.steps += 1
        if self.config["safety_guarantee"]:
            self.new_action = self.safety_supervisor(action, adv_action)
        else:
