...
class MAA2C(Agent):
    ...
    def evaluation(self, env, output_dir, eval_episodes=1, is_train=True, render = False):
        rewards, regional_rew = [], []
        infos = []
        avg_speeds = []
        steps = []
        vehicle_speed = []
        ...
        for i in range(eval_episodes):
            ...
            while not done:
                ...
                pre = [y.position[0] for y in env.env.road.vehicles]
                # print("steps:",step,"actions:",action)
                # print(state)
                state, reward, done, info = env.step(action)
                after = [y.position[0] for y in env.env.road.vehicles]
                ...
                infos_i.append(info)
                regional_rew_i.append(sum(info["regional_rewards"])/len(env.controlled_vehicles))

            vehicle_speed.append(info["vehicle_speed"])
            vehicle_position.append(info["vehicle_position"])
            rewards.append(rewards_i)
            regional_rew.append(regional_rew_i)
            infos.append(infos_i)
            steps.append(step)
            avg_speeds.append(avg_speed / step)

            only_advs.append(env.only_adv_crashes()) 
            only_cavs.append(env.only_cavs_crashes()) 

        if render and video_recorder is not None:
            video_recorder.release()
        env.close()
        return rewards, regional_rew, (vehicle_speed, vehicle_position), steps, avg_speeds, only_advs, only_cavs
