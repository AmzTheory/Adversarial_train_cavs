...
from highway_env.vehicle.controller import MDPVehicle, CAVehicle
...
class AbstractEnv(gym.Env):
    ...
    def default_config(cls) -> dict:
        ...
        return {
            ...
            "other_vehicles_type": "highway_env.vehicle.controller.CAVehicle",
    ...
    def reset(self, is_training=True, testing_seeds=0, num_CAV=0) -> Observation:
        ...
        if self.config["action_masking"]:
            available_actions = self._get_mask([self.vehicle])
        else:
            available_actions = [1] * self.n_a 
        return np.asarray(obs).reshape(-1), np.array(available_actions)
    ...
    def safety_supervisor(self, actions, adv_act=0):
        """"
        ...
        index = 0
        """set the priotity scores"""
        for vehicle, action in zip(env_copy.CAVs, actions):
        ...
        """make list contain items order according to priority"""
        ...
        for i, vehicle_and_action in enumerate(vehicles_and_actions):
            ...
            if len(vehicle_and_action[0].trajectories) == n_points:
                ...
                env_copy.CAVs[index] = copy.deepcopy(self.CAVs[index])
                vehicle = env_copy.CAVs[index]
            ...
            """obtain the list of neighbouring vehicles"""
            ...
            for t in range(n_points):
                ...
                for v in [v_fl, v_fr, vehicle, v_rl, v_rr]:
                    ...
                    # other surrounding vehicles
                        ...
                        elif type(v) is MDPVehicle:
                            mdp_controller(v, env_copy,  adv_act)
                        elif type(v) is CAVehicle   and v is not vehicle:
                            # use the previous action: idle
                            mdp_controller(v, env_copy,  actions[v.id])
                        elif type(v) is CAVehicle  and v is vehicle:
                ...
                """replace action in the an incident of crash"""
                if vehicle.crashed:
                    ...
                    for a in available_actions:
                        vehicle_copy = copy.deepcopy(self.CAVs[index])
                    ...
                    env_copy.CAVs[index] = updated_vehicles[safety_rooms.index(max(safety_rooms))]
                    vehicle = env_copy.CAVs[index]
                    env_copy.road.vehicles[index] = vehicle
                    if first_change:
                        first_change = False
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        ## all CAVs decide on the next action using the model
        model_state =  np.zeros(shape= (len(self.CAVs), 5, 5))
        for i, cav in enumerate(self.CAVs):
            state = cav.get_state()
            model_state[i] = state 
            # print("id =",str(cav.id))
            # print("state =",str(state.reshape(1,-1)))   
            # print("##########################")    
        masks = self._get_mask(self.CAVs)
        model = self.config["cav_model"]

        model_state = model_state.reshape(len(self.CAVs), -1)
        
        predicted_actions = model.action(model_state, len(self.CAVs), masks)

        if self.config["safety_guarantee"]:
            predicted_actions = list(self.safety_supervisor(predicted_actions, adv_act=action))
        # predicted_actions = [1] * len(self.CAVs)
        for i in range(len(predicted_actions)):
            self.CAVs[i].set_action(predicted_actions[i])
        
        # print("##########"+ str(self.steps) +"##########")
        # print([cav.id for cav in self.CAVs])
        # print(predicted_actions)
        # action is a tuple, e.g., (2, 3, 0, 1)
        # print("steps:",self.steps,"actions:",predicted_actions)
        # print(model_state)
        self._simulate(self.new_action)
        # print("positions",str([ (round(a,2),round(b,2)) for a,b in zip(pre,after)]))
        # print("###############################")
    ...
    def _get_mask(self, vehicles):
        if self.config["action_masking"]:
            available_actions = [[0] * self.n_a for _ in range(len(vehicles))]
            for i in range(len(vehicles)):
                available_action = self._get_available_actions(vehicles[i], self)
                for a in available_action:
                    available_actions[i][a] = 1
        else:
            available_actions = [[1] * self.n_a] * len(vehicles)

        return available_actions[0] if len(vehicles) == 1 else available_actions
